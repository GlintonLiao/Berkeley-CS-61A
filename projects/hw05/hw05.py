class VendingMachine:
    ...
    def add_funds(self, fund_num):
        if self.restore == 0:
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(fund_num)
        else:
            self.balance += fund_num
            return 'Current balance: ${0}'.format(self.balance)
    
    def restock(self, restock_num):
        self.restore += restock_num
        return 'Current {0} stock: {1}'.format(self.name, self.restore)
