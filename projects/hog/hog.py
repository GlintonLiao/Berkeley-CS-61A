...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i <= num_rolls:
        dice_num = dice()
        if dice_num != 1 and sum_num != 1:
            sum_num = sum_num + dice_num
        else:
            sum_num = 1
        i += 1
    return sum_num
...
def swine_align(player_score, opponent_score):
    ...
    while i <= min(player_score, opponent_score):
        if player_score % i == opponent_score % i == 0:
            factor = i
            i += 1
        else:
            i += 1
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def running_aver(*args):
        i, sum_of_call = 1, 0
        while i <= trials_count:
            sum_of_call += original_function(*args)
            i += 1
        average_sum = sum_of_call / trials_count
        return average_sum
    return running_aver
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while i >= 1:
        ...
        if current_highest - average_score <= 0:
            current_highest = average_score
            num_dice = i
            i -= 1
        else:
            i -= 1
