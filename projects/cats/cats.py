...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0 or len(reference_words) == 0:
        ...
        num_correct, num_type = 0, len(typed_words)
        i = 0
        while i < min(len(reference_words), len(typed_words)):
            if typed_words[i] == reference_words[i]:
                ...
                i += 1
        return num_correct / num_type * 100
    # END PROBLEM 3
...
def pawssible_patches(start, goal, limit):
    ...
    if limit < 0: # Fill in the condition
        ...
        diff = start[0] != goal[0] #条件判断 True是1，False是0
...
def report_progress(typed, prompt, user_id, send):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    ratio_correct = num_correct / len(prompt)
    send({'id': user_id, 'progress': ratio_correct})
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        for k in range(1, len(times_per_player[i])):
            time_new += [times_per_player[i][k] - times_per_player[i][k - 1]]
        time_elapsed.append(time_new)
        time_new = [] #循环后，让它重新变成空集
    return game(words, time_elapsed)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for i in player_indices:
        time_list += [[]] #做一个包含各个选手的空集
    for k in word_indices:
        for n in player_indices:
            if time(game, n, k) == min(time(game, n, k) for n in player_indices):
                time_list[n] += [word_at(game, k)]
                break #如果是最快的单词了，就跳出这一次循环， which is 开始下一个单词（从line246重新开始）
    return time_list
